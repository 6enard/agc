      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid gap-4 sm:gap-6 lg:gap-8 mb-12 sm:mb-16 lg:mb-20 lg:grid-cols-2">
          {services.map((service, index) => (
            <div key={index} className="bg-gradient-to-br from-blue-600 to-blue-700 rounded-xl sm:rounded-2xl p-4 sm:p-6 lg:p-8 text-white relative overflow-hidden">
              ...
              <div className="relative z-10">
                ...
                <p className="text-blue-100 mb-4 sm:mb-6 text-sm sm:text-base lg:text-lg">{service.description}</p>
                <div className="space-y-1 sm:space-y-2">
                  {service.features.map((feature, idx) => (
                    <div key={idx} className="flex items-center">
                      <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-blue-300 rounded-full mr-2 sm:mr-3 flex-shrink-0"></div>
                      <span className="text-blue-100 text-xs sm:text-sm lg:text-base">{feature}</span>
                    </div>
                  ))}
                </div>
              </div>
